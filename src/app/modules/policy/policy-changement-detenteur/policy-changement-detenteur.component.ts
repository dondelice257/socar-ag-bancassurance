...
@Component({
  selector: 'app-policy-changement-detenteur',
  standalone: true,
  imports: [
    ReactiveFormsModule, 
    LookupComponent
  ],
  templateUrl: './policy-changement-detenteur.component.html',
  styleUrl: './policy-changement-detenteur.component.scss'
})
export class PolicyChangementDetenteurComponent {
  renewForm:FormGroup
  ...
  constructor(
    ...
    this.renewForm = this.fb.group({
      new_client: ['', [Validators.required]],
  ...
  changementDetenteur(): void {
    this.isLoading = true;
  
    this.policyService.doPolicyActionForOnlyRenew(this.policyId, 'change_detenteur', this.renewForm.value)
      .subscribe({
        next: (data: any) => {
  ...
  onPolicySelected(policy: any) {
    ...
    this.renewForm.patchValue({new_client: this.selectedClientId});
  }
